...
class CapCompareQuotationService extends cds.ApplicationService {
  async init() {
    ...
    const S4_QUOTATION_COMPARISON_SRV = await cds.connect.to(
      "S4_API_QUOTATION_COMPARISON",
    );
    this.on("READ", "A_RequestForQuotation", (req) =>
      S4_RFQ_PROCESS_API.run(req.query),
    );
    this.on("READ", "A_RequestForQuotationItem", (req) =>
      S4_RFQ_PROCESS_API.run(req.query),
    );
    this.on("READ", "SupplierQuotation", (req) =>
      this.readSupplierQuotation(req),
    );
    this.on("READ", "SupplierQuotationItem", (req) =>
      this.readSupplierQuotationItem(req),
    );

    this.on("READ", "QuotationComparison", (req) =>
      this.readQuotationComparison(req),
    );
    this.on("READ", "QuotationComparisonItem", (req) =>
      this.readQuotationComparisonItem(req),
    );
    this.on("READ", "TermsAndConditions", (req) =>
      this.readTermsAndConditions(req),
    );

    // this.on('READ', 'RFQs', req => this.readRFQsAndNavigationData(req));
    this.on(
      "upsertCompareQuotation",
      async ({
        data: { quotationComparison, quotationComparisonItem, type },
      }) =>
        this.upsertCompareQuotation(
          quotationComparison,
          quotationComparisonItem,
          type,
        ),
    );
    this.on("workflowForCompareQuotation", async (req) =>
      this.workflowForCompareQuotation(req),
    );
    return super.init();
  }
  ...
  entitySetAvailableInUrl(from, sEntitySetName) {
    ...
    return (
      Array.isArray(from?.ref) && from.ref.some((r) => r === sEntitySetName)
    );
  }
